...
async def eval_python(event: hikari.MessageCreateEvent):
    try:
        ...
        if code_block.lang == None:
            ...
            await event.message.respond("Only python is supported", reply=True, mentions_reply=False)
            return
        ...
        def cleanup():
            try:
                ...
                asyncio.run_coroutine_threadsafe(event.message.respond(f"**Failed to kill all processes created by snippet**: `{e}`"), asyncio.get_event_loop())
                return
        ...
        with ExitStack() as stack:
            ...
            try:
                ...
                response = f"**Process timed out (timeout is {timeout} seconds)**"
                if e.output != None and len(e.output.strip()) != 0:
                    response += f"\n```\n{e.output.decode('utf8').strip()}\n```"
                await event.message.respond(response, reply=True)
                return
        ...
        await event.message.respond(f"**Encountered unexpected error**: `{e}`")
